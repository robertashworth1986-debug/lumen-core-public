...
def test_valuation_packet_uses_real_replay_numbers_and_selects_energy_target():
    ...
    assert payload["schema"] == "valuation_proposal_target_packet_v2"
    assert overall["source_conditioned_route_count"] >= 10
    assert overall["candidate_win_count"] >= 1
    assert overall["estimated_rows_replayed"] > 0
    assert overall["numeric_samples_read"] > 0
    assert overall["comparison_win_rate"] == round(
        overall["candidate_win_count"] / overall["baseline_comparison_count"],
        6,
    )

    assert target["strongest_lane"] == "wave_resonance_timing"
    assert target["strongest_candidate"] == "kuramoto_phase_coupling"
    assert target["strongest_candidate_label"] == "Kuramoto phase coupling"
    assert valuation["defensible_money_status"].lower().startswith("price paid technical evaluation")
    assert target["target_segment"].startswith("utility_grid_analytics_energy_forecasting")
...
def test_valuation_packet_marks_cumberland_claims_ahead_of_gate_and_blocks_hype():
    ...
    assert "Do not state field validation, realized savings" in rendered
