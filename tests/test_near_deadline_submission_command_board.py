...
def test_near_deadline_board_identifies_stage_now_and_human_gates():
    ...
    assert "4/10" in payload["summary"]["strongest_today_action"]
    assert "6 open" in payload["summary"]["strongest_today_action"]
    assert "Nashville EC is portal-confirmed" in payload["summary"][
        "strongest_today_action"
    ]
    assert "4/10 gates pass" in payload["summary"]["fastest_low_friction_lane"]
    ...
    assert build_week["readiness_gate_pass_count"] == 4
    assert build_week["readiness_gate_open_count"] == 6
    assert build_week["public_demo_url"] is None
...
def test_near_deadline_board_rendering_is_safe_and_cites_sources():
    ...
    assert "4/10 gates pass" in rendered
