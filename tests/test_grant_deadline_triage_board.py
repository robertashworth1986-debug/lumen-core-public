...
def test_deadline_triage_uses_official_dice_and_dsip_dates():
    ...
    assert board["official_deadlines"]["dsip"]["proposal_window_boundary"].startswith("Public topic rows show pre-release")
...
def test_deadline_triage_preserves_submit_and_evidence_boundaries():
    ...
    assert live_gate["active_start_package"] == "DICE"
    assert live_gate["active_start_deadline_utc"] == "2026-06-30T18:00:00+00:00"
    assert live_gate["closest_action_gate_portal"] == "DSIP"
    assert live_gate["closest_action_gate_utc"] == "2026-06-24T12:00:00+00:00"
    ...
    assert "TPOC Q&A closing June 24, 2026" in rendered
