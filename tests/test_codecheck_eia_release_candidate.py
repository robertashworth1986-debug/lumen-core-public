...
def test_bundle_has_fixed_metadata_exact_inputs_and_no_duplicate_names(tmp_path):
    ...
    with zipfile.ZipFile(bundle, "r") as archive:
        ...
        assert f"{root}/code/ops/VERIFY_CODECHECK_MAINLINE_INTEGRATION.py" in names
        assert f"{root}/config/codecheck_eia_mainline_integration_v1.json" in names
        assert f"{root}/tests/test_codecheck_reviewer_container.py" in names
        assert (
            f"{root}/evidence/reproducibility/"
            "codecheck_reviewer_container_1c0eb517_20260721/runtime_receipt.json"
            in names
        )
