...
LOCKED_SWEEP = ROOT / "dashboard" / "data" / "locked_source_baseline_replay_sweep.json"
...
def test_metric_battery_consolidates_champion_without_overclaiming():
    ...
    sweep_summary = json.loads(LOCKED_SWEEP.read_text(encoding="utf-8"))["summary"]
    ...
    assert summary["locked_sweep_estimated_rows_replayed"] == sweep_summary["estimated_rows_replayed"]
    assert summary["locked_sweep_baseline_comparison_count"] == sweep_summary["baseline_comparison_count"]
    ...
    assert summary["locked_sweep_manifest_source_count"] == sweep_summary["source_count"]
...
def test_metric_battery_names_real_test_lanes_and_blockers():
    ...
    assert (
        lanes["source_generalization"]["evidence"]["locked_sweep_baseline_comparison_count"]
        == sweep_summary["baseline_comparison_count"]
    )
    assert lanes["source_generalization"]["evidence"]["locked_sweep_source_system_count"] >= 8
    assert "leave-one-source-out" in lanes["source_generalization"]["next_action"]
