...
def test_contact_identity_gate_requires_company_domain_mailbox(profile: dict) -> None:
    profile["company"]["website"] = "https://lumen-core.ai"
    profile["company"]["email"] = "candidate@gmail.com"

    blocked, reason = factory._contact_identity_gate(profile)

    assert blocked is False
    assert "generic inbox" in reason

    profile["company"]["email"] = "candidate@lumen-core.ai"
    allowed, allowed_reason = factory._contact_identity_gate(profile)

    assert allowed is True
    assert "matches" in allowed_reason
