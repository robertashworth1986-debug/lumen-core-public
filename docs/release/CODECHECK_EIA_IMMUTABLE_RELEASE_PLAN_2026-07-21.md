...
| Commit-pinned source and preprint reference | `present at 1c0eb517` | GitHub |
| Version DOI | `not assigned; optional strengthening` | Zenodo |
...
1. Robert reviews the frozen preprint, six-file manifest, claim boundary, and author checklist.
2. Reconcile PR overlap and run the current-head reviewer workflow without adding unrelated implementations.
3. Reconcile Gmail, GitHub, and local controls for any prior CODECHECK request.
4. Confirm the CODECHECK Launch Pad assigns one collision-free identifier.
5. Obtain fresh action-time HumanUnlock before opening exactly one production CODECHECK request.
6. Separately decide whether to create an immutable GitHub release and Zenodo DOI as provenance strengthening; neither is treated as external validation.
7. If approved, build the five release assets and reconcile them against `SHA256SUMS` before any draft release is created.
8. Obtain fresh action-time HumanUnlock before creating or publishing a release or changing a repository integration.
9. Record only observed release URLs, hashes, or DOI values. Do not infer them from a pending integration.
