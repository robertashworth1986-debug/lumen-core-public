...
Because `dashboard/evidence/index.html` is a static asset, the bounded recovery target is a static Nginx route. This avoids making the public evidence landing page depend on the application gateway.
...
The public page must remain run-scoped. Restoring the route is not external validation, agency approval, field performance, universal model superiority, or guaranteed savings.
...
The default mode is read-only. It checks for the Nginx file and static page, then prints the proposed diff. A nonzero `NEEDS_REPAIR` result means no file was changed.
...
1. creates a timestamped rollback copy;
2. replaces only the existing `/evidence/` location block;
3. creates a second timestamped backup through the Python repair utility;
4. runs `nginx -t`;
5. restores the rollback copy if Nginx validation fails;
6. reloads Nginx only after validation succeeds;
7. checks the local and public `/evidence/` status.
...
- `/evidence/` returns HTTP `200` from the static dashboard tree;
