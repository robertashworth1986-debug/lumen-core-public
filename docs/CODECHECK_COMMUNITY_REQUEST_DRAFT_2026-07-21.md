...
Status: `HOLD_STABLE_PREPRINT_IDENTIFIER_AND_HUMANUNLOCK`
...
1. Publish or identify a stable public preprint or article URL/DOI.
2. Freeze an immutable public source release or commit and verify every packet hash against it.
3. Reconcile Gmail, GitHub, and local outreach registries to prove no duplicate CODECHECK request exists.
4. Robert reviews the public manuscript, issue body, source release, license, and bounded ask.
5. Obtain a fresh action-time HumanUnlock from Robert for exactly one production-register issue.
6. Open the issue through the current production Launch Pad.
