...
| AC-05 | Dependency inventory and SBOM | Partial or scoped | The reviewer computation has a scoped inventory; current named deployed release `1ce7c359` has deterministic 43-file CycloneDX 1.6 coverage and a constrained, successfully verified signed SBOM. | Inventory VPS/runtime layers and add vulnerability triage, exception handling, and periodic trusted-root re-verification. |
| AC-06 | Build provenance and release integrity | Partial or scoped | Static release files bind to Git identity and SHA-256; current named release `1ce7c359` binds the exact archive to constrained GitHub OIDC/Sigstore predicates, human-gated deployment, rollback capture, 43-of-43 live-byte verification, and a read-only post-deployment audit; prior `e513f65a` remains in the append-only receipt history. | Assess any SLSA level separately, extend provenance to deployment runtime layers, and repeat the exact release gate for every later commit. |
