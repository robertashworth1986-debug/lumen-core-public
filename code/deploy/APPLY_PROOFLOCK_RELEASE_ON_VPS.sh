...
[[ "$approval" == "$REQUIRED_APPROVAL" ]] || die "explicit ProofLock deployment approval is required"
...
test_mode=0
if [[ "${PROOFLOCK_DEPLOY_TEST_MODE:-}" == "1" ]]; then
  [[ "$EUID" -ne 0 ]] || die "test mode is forbidden for root"
  [[ -n "${PROOFLOCK_DEPLOY_TEST_ROOT:-}" ]] || die "test mode requires PROOFLOCK_DEPLOY_TEST_ROOT"
  [[ "$PROOFLOCK_DEPLOY_TEST_ROOT" == /* && "$PROOFLOCK_DEPLOY_TEST_ROOT" != "/" ]] || \
    die "test root must be a non-root absolute path"
  target_root="$PROOFLOCK_DEPLOY_TEST_ROOT/opt/lumencore/dashboard/build_week/prooflock_console"
  rollback_base="$PROOFLOCK_DEPLOY_TEST_ROOT/opt/lumencore/rollbacks/prooflock"
  test_mode=1
else
  [[ -z "${PROOFLOCK_DEPLOY_TEST_ROOT:-}" ]] || die "test root is forbidden outside test mode"
...
root_stage="$(mktemp -d /tmp/lumencore-prooflock-root.XXXXXXXX)"
rollback_dir=""
...
cleanup() {
  if [[ -n "$root_stage" && "$root_stage" == /tmp/lumencore-prooflock-root.* ]]; then
    rm -rf -- "$root_stage"
  fi
}
...
restore_directory_state() {
  ...
  local index path state uid gid mode actual_uid actual_gid actual_mode
  local restore_failed=0
  for ((index = ${#directory_rows[@]} - 1; index >= 0; index--)); do
    IFS=$'\t' read -r path state uid gid mode <<<"${directory_rows[$index]}"
    [[ "$path" != "path" ]] || continue
    if [[ "$state" == "PRESENT" ]]; then
      if [[ -d "$path" && ! -L "$path" ]]; then
        ...
        [[ "$actual_mode" == "$mode" ]] || restore_failed=1
        if [[ "$test_mode" -eq 0 ]]; then
          [[ "$actual_uid" == "$uid" && "$actual_gid" == "$gid" ]] || restore_failed=1
        fi
      else
        restore_failed=1
      fi
  ...
  if [[ "$deployment_started" -eq 1 && -n "$rollback_dir" ]]; then
    ...
    for name in "${RELEASE_FILES[@]}"; do
      ...
      if [[ -f "$backup_path" && ! -L "$backup_path" ]]; then
        ...
        read -r target_uid target_gid target_mode < <(stat -c '%u %g %a' -- "$target_path")
        [[ "$target_uid" == "$backup_uid" && "$target_gid" == "$backup_gid" ]] || rollback_failed=1
        [[ "$target_mode" == "$backup_mode" ]] || rollback_failed=1
...
repo_prefix = "build_week/prooflock_console/"
...
for row in rows:
    if not isinstance(row, dict):
        fail("manifest file row must be an object")
...
with tarfile.open(archive_path, mode="r:") as archive:
    ...
    for member in members:
        ...
        if row.get("repo_path") != repo_prefix + member.name:
...
[[ -d "$dashboard_root" && ! -L "$dashboard_root" ]] || die "dashboard root must be a real directory"
[[ "$(realpath -m -- "$dashboard_root")" == "$dashboard_root" ]] || \
  die "dashboard root cannot traverse a symlink"
for path in "$build_week_root" "$target_root"; do
...
[[ "$(realpath -m -- "$rollback_base")" == "$rollback_base" ]] || \
  die "rollback path cannot traverse a symlink"
...
for path in "$build_week_root" "$target_root"; do
  if [[ -d "$path" ]]; then
    ...
    printf '%s\tMISSING\t-\t-\t-\n' "$path" >> "$rollback_dir/directory-state.tsv"
  fi
done
...
for name in "${RELEASE_FILES[@]}"; do
  ...
  if [[ -L "$target_path" ]]; then
    die "target file cannot be a symlink: $target_path"
    ...
    printf '%s\tPRESENT\t%s\t%s\t%s\t%s\t%s\t%s\n' \
      "$name" "$current_hash" "$owner" "$group" "$uid" "$gid" "$mode" \
      >> "$rollback_dir/pre-deploy.tsv"
    cp -a -- "$target_path" "$rollback_dir/files/$name"
  elif [[ -e "$target_path" ]]; then
    die "target path has an unsupported type: $target_path"
  else
    printf '%s\tMISSING\t-\t-\t-\t-\t-\t-\n' "$name" \
      >> "$rollback_dir/pre-deploy.tsv"
  fi
...
if [[ "$test_mode" -eq 1 ]]; then
  install -d -m 0755 -- "$build_week_root" "$target_root"
else
  install -d -o root -g root -m 0755 -- "$build_week_root" "$target_root"
fi

for name in "${RELEASE_FILES[@]}"; do
  source_path="$root_stage/unpacked/$name"
  temporary_target="$target_root/.${name}.prooflock-${timestamp}-$$"
  ...
  [[ "$temporary_hash" == "${expected_hashes[$name]}" ]] || \
    die "staged target hash mismatch: $name"
done
...
for name in "${RELEASE_FILES[@]}"; do
  target_paths+=("$target_root/$name")
done
chmod 0755 -- "$build_week_root" "$target_root"
chmod 0644 -- "${target_paths[@]}"
if [[ "$test_mode" -eq 0 ]]; then
  chown root:root -- "$build_week_root" "$target_root"
  chown root:root -- "${target_paths[@]}"
fi
...
for name in "${RELEASE_FILES[@]}"; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$name" "${expected_hashes[$name]}" "$actual_hash" "$owner" "$group" "$uid" "$gid" "$mode" \
    >> "$rollback_dir/post-deploy.tsv"
done

deployment_started=0
printf 'PROOFLOCK_SOURCE_COMMIT=%s\n' "$source_commit"
printf 'PROOFLOCK_ROLLBACK_DIR=%s\n' "$rollback_dir"
printf '%s\n' '--- remote pre-deploy identity ---'
cat "$rollback_dir/pre-deploy.tsv"
printf '%s\n' '--- exact post-deploy identity ---'
cat "$rollback_dir/post-deploy.tsv"
printf '%s\n' 'PROOFLOCK_DEPLOYMENT_OK'
