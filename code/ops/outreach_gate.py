...
This tool never sends email. It validates the shared outreach registry and
decides whether a proposed draft or send action is permitted.
"""
...
def validate_registry(payload: dict[str, Any]) -> None:
    ...
    cooldown = policy["default_cooldown_hours"]
    ...
    for index, campaign in enumerate(campaigns):
        ...
        sequence = campaign["outbound_sequence"]
        if not isinstance(sequence, int) or sequence < 0:
            raise RegistryError(
                f"campaigns[{index}].outbound_sequence must be "
        ...
        count = campaign["duplicate_count"]
        if not isinstance(count, int) or count < 0:
            raise RegistryError(
                f"campaigns[{index}].duplicate_count must be "
                "a non-negative integer"
            )
        if campaign["duplicate_detected"] != (count > 0):
            raise RegistryError(
                f"campaigns[{index}] duplicate flag/count are inconsistent"
            )
        if not isinstance(campaign["inbound_since_last_outbound"], bool):
            raise RegistryError(
                f"campaigns[{index}].inbound_since_last_outbound "
                "must be boolean"
            )
...
def evaluate(
    ...
    *,
    ...
    if actor == "human" and not explicit_approval:
        reasons.append("human send must record explicit approval for the gate")
