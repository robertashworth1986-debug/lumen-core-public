...
def table(headers: list[str], rows: list[list[str]], caption: str) -> str:
    ...
    table_class = ' class="wide-table"' if len(headers) >= 4 else ''
    return f'<div class="table-scroll" tabindex="0" role="region" aria-label="{esc(caption)}"><table{table_class}><caption>{esc(caption)}</caption><thead><tr>{head}</tr></thead><tbody>{body}</tbody></table></div>'
