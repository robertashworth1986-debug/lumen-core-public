...
def _process_is_alive(pid: int) -> bool:
    ...
    if os.name == "nt":
        ...
        # query-only process handle so lock recovery can never signal or
        # terminate the process whose liveness it is checking.
        import ctypes
        ...
        if not handle:
            ...
            # Other failures, including access denial, fail closed as alive.
...
def _is_stale_lock(
    ...
    if _lock_age_seconds(lock_path, payload) < stale_after_seconds:
        return False
    if not payload:
        return False
    ...
    if owner_host != socket.gethostname():
        # A local process table cannot prove a remote owner dead. Shared-path
        # residue therefore requires operator review instead of auto-reclaim.
        return False
...
def exclusive_append_lock(
    ...
    *,
    ...
    """Acquire an ownership-token lock and reclaim only provably stale residue.

    The lock file is created atomically. Crash residue can be reclaimed only
    after the stale threshold and only when its recorded local process is no
    longer alive. Cleanup verifies the owner token, so an exiting process never
    removes a successor's lock.
    """
        ...
        try:
            ...
            if time.monotonic() >= deadline:
                if callable(error_message):
                    ...
                    message = error_message or f"timed out waiting for append lock: {path.name}"
